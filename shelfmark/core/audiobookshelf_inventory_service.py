...
during metadata search to flag audiobooks/series already present in the user's
library. Mirrors the Kavita inventory service.
"""
...
_CREATE_SQL = """
CREATE TABLE IF NOT EXISTS abs_inventory (
...
CREATE INDEX IF NOT EXISTS idx_abs_inventory_isbn13 ON abs_inventory (isbn_13);
CREATE INDEX IF NOT EXISTS idx_abs_inventory_isbn10 ON abs_inventory (isbn_10);
CREATE INDEX IF NOT EXISTS idx_abs_inventory_title_author
ON abs_inventory (norm_title, norm_author);
CREATE INDEX IF NOT EXISTS idx_abs_inventory_series ON abs_inventory (norm_series);
CREATE INDEX IF NOT EXISTS idx_abs_inventory_series_index
ON abs_inventory (norm_series, series_index);
CREATE INDEX IF NOT EXISTS idx_abs_inventory_series_full
ON abs_inventory (norm_series_full, series_index);
"""


class AbsInventoryService:
    ...
    def replace_inventory(self, records: Iterable[dict[str, Any]]) -> dict[str, Any]:
        ...
        Records removed from Audiobookshelf disappear because the table is fully
        rebuilt in one transaction. Returns ``{"rows", "new_books", "baseline"}``
        where ``new_books`` are records whose identity was absent from the prior
        ...
        with self._lock, self._connect() as conn:
            ...
            conn.executemany(
                """
                INSERT INTO abs_inventory
                    (kind, library_id, abs_series_id, series_name, title, author,
        ...
        logger.info(
            "Audiobookshelf inventory snapshot replaced: %d rows (%d new books%s)",
            len(rows),
            len(new_books),
    ...
    def count(self) -> int:
        ...
        try:
            with self._lock, self._connect() as conn:
                cur = conn.execute("SELECT COUNT(*) FROM abs_inventory")
        ...
        *,
        ...
        try:
            with self._lock, self._connect() as conn:
                ...
                if clean_13:
                    cur = conn.execute(
                        "SELECT 1 FROM abs_inventory WHERE isbn_13 = ? LIMIT 1",
                        (clean_13,),
                    )
                    if cur.fetchone():
                        return True
                ...
                if clean_10:
                    cur = conn.execute(
                        "SELECT 1 FROM abs_inventory WHERE isbn_10 = ? LIMIT 1",
                        (clean_10,),
                    )
                    if cur.fetchone():
                        return True
...
_service: AbsInventoryService | None = None
_service_lock = threading.Lock()


def init_abs_inventory_service(db_path: str) -> AbsInventoryService:
    """Create (once) and return the process-wide ABS inventory service singleton."""
    global _service
    with _service_lock:
        if _service is None:
            _service = AbsInventoryService(db_path)
            _service.initialize()
        return _service


def get_abs_inventory_service() -> AbsInventoryService | None:
    """Return the ABS inventory service if it has been initialized, else None."""
    return _service
