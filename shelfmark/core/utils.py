...
def get_named_download_destinations(
    *,
    user_id: int | None = None,
    username: str | None = None,
) -> list[dict[str, str]]:
    """Named, user-selectable download destinations for the per-download picker.

    Returns Shelfmark's built-in typed folders (Ebooks, and Audiobooks when a
    separate audiobook folder is configured) followed by every configured
    library folder, each as ``{"id", "label", "path"}``. The "Automatic (by
    type)" default is intentionally omitted — the UI adds it as the first
    option, and an empty/absent selection preserves today's routing.
    """
    from shelfmark.core.config import config
    ...
    def _add(dest_id: str, label: str, raw_path: object) -> None:
        if dest_id in seen_ids:
            return
        expanded = _expand_user_destination_placeholder(
            str(raw_path or ""),
            user_id=user_id,
            username=username,
        ).strip()
        if not expanded:
            return
        # Normalize away trailing slashes (e.g. /books/{user} with no user → /books/).
        # Path("") would become "." so guard that case explicitly.
        try:
            path = str(Path(expanded))
        except (TypeError, ValueError):
            return
        if not path or path == ".":
            return
        seen_ids.add(dest_id)
        destinations.append({"id": dest_id, "label": label, "path": path})

    ebooks = config.get("DESTINATION", "", user_id=user_id) or config.get("INGEST_DIR", "")
    _add("ebooks", "Ebooks", ebooks)
    _add("audiobooks", "Audiobooks", config.get("DESTINATION_AUDIOBOOK", "", user_id=user_id))

    for folder in get_library_folders():
        # Use the folder path (not name) as the ID so two folders with the same
        # name both appear in the picker rather than the second being silently dropped.
        _add(f"lib:{folder['path']}", folder["name"], folder["path"])

    return destinations


def resolve_named_download_destination(
    dest_id: object,
    *,
    user_id: int | None = None,
    username: str | None = None,
) -> str | None:
    """Resolve a destination id from the picker to its absolute path.

    Returns None for the automatic default, an unknown id, or a blank value, so
    callers fall back to content-type routing. Resolving server-side from the
    known destination list means a client can never steer a download to an
    arbitrary path.
    """
    chosen = str(dest_id or "").strip()
    if not chosen or chosen == "auto":
        return None
    for dest in get_named_download_destinations(user_id=user_id, username=username):
        if dest["id"] == chosen:
            return dest["path"]
    return None
