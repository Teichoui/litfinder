"""APScheduler-based scheduling for the Calibre-Web / CWA inventory sync.
...
from shelfmark.integrations.calibre.sync import run_calibre_sync

logger = setup_logger(__name__)

DEFAULT_CRON = "0 * * * *"
_CRON_JOB_ID = "calibre_sync_cron"
_DOWNLOAD_JOB_ID = "calibre_sync_after_download"
...
def _ensure_scheduler() -> BackgroundScheduler:
    ...
    if _scheduler is None:
        ...
        logger.info("Calibre scheduler started")
    return _scheduler
...
def _apply_cron(scheduler: BackgroundScheduler, *, enabled: bool, expression: str) -> None:
    ...
    if not enabled:
        if existing is not None:
            ...
            logger.info("Calibre cron sync disabled; job removed")
        return
    ...
    try:
        ...
        logger.warning("Invalid Calibre cron '%s'; falling back to '%s'", expression, DEFAULT_CRON)
    ...
    logger.info("Calibre cron sync scheduled: '%s'", expression)
...
def start_scheduler() -> None:
    ...
    with _lock:
        ...
        _apply_cron(
            scheduler,
            enabled=coerce_bool(config.get("CALIBRE_SYNC_ENABLED", False)),
            expression=str(config.get("CALIBRE_SYNC_CRON", DEFAULT_CRON) or DEFAULT_CRON),
        )
...
def request_sync_after_download() -> None:
    ...
    Calibre-Web / CWA time to ingest the new file before we re-scan.
    """
    if not coerce_bool(config.get("CALIBRE_SYNC_ON_DOWNLOAD", False)):
        return
    with _lock:
        scheduler = _ensure_scheduler()
    ...
    logger.debug("Calibre post-download sync scheduled in %ds", _DOWNLOAD_DEBOUNCE_SECONDS)
