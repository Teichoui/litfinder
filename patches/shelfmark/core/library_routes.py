"""Library management API routes for file organization."""
...
from shelfmark.core.abs_integration import ABSError, get_abs_client, refresh_library_for_path
from shelfmark.core.logger import setup_logger

logger = setup_logger(__name__)

_MEDIA_EXTENSIONS = {".m4b", ".mp3", ".m4a", ".flac", ".wav", ".aac", ".epub", ".pdf", ".mobi", ".azw", ".azw3"}
...
def register_library_routes(app: Flask, *, resolve_auth_mode: Callable[[], str]) -> None:
    """Register library management routes on the Flask app."""
    ...
    def api_library_browse() -> Response | tuple[Response, int]:
        ...
        try:
            ...
            logger.error("Failed to browse ABS libraries: %s", exc)
            return jsonify({"error": "Failed to connect to Audiobookshelf"}), 502

    @app.route("/api/library/<library_id>/items", methods=["GET"])
    def api_library_items(library_id: str) -> Response | tuple[Response, int]:
        if (gate := _require_auth("browse")) is not None:
            return gate
        ...
        try:
            return jsonify(client.get_library_items(library_id, limit=limit, page=page))
        except ABSError as exc:
            logger.error("Failed to get items for library %s: %s", library_id, exc)
            return jsonify({"error": "Failed to get library items"}), 502
    ...
    def api_library_move_file() -> Response | tuple[Response, int]:
        ...
        for _p in {str(source.parent), str(dest.parent)}:
            threading.Thread(target=refresh_library_for_path, args=(_p,), daemon=True).start()
    ...
    def api_library_scan() -> Response | tuple[Response, int]:
        if (gate := _require_auth("scan")) is not None:
            return gate
        ...
        if client.scan_library(library_id):
            return jsonify({"success": True, "message": "Library scan triggered"})
        return jsonify({"error": "Failed to trigger library scan"}), 500
    ...
    def api_library_organize() -> Response | tuple[Response, int]:
        ...
        if not _is_within_allowed(dest_dir, allowed):
            return jsonify({"error": "target_folder is not within a configured library folder"}), 403
        ...
        for file_info in files:
            ...
            if not _is_within_allowed(source, allowed):
                failed.append({"path": source_path, "error": "Not within a configured library folder"})
                continue
            ...
            try:
                ...
                scanned_dirs.add(str(source.parent))
            except (OSError, shutil.Error) as exc:
                logger.error("Failed to move %s: %s", source_path, exc)
                failed.append({"path": source_path, "error": str(exc)})

        # Scan each unique affected directory once, not once per file
        scanned_dirs.add(str(dest_dir))
        for dir_path in scanned_dirs:
            threading.Thread(target=refresh_library_for_path, args=(dir_path,), daemon=True).start()
    ...
    def api_library_ls() -> Response | tuple[Response, int]:
        ...
        try:
            ...
            logger.error("Failed to list directory %s: %s", folder_path, exc)
    ...
    def api_library_rename() -> Response | tuple[Response, int]:
        ...
        threading.Thread(target=refresh_library_for_path, args=(str(src.parent),), daemon=True).start()
